	...
	if int(FEAT[3]) == 1:
		for line in mat :
			if "DEPENDENCY" in line :
				nDep += 1
	print "  Thre are %s dependencies within the matrix"%(nDep)
	### taille du motif
	size = 0
	if nDep > 0 :
...
############
def SeqToDensities(fasta,mat,sym,output,matname,name=""):
...
#	fileScores.write("name\tposition\tsens\tscore\n")
	...
	print "  Il y a %s séquences à analyser"%(len(sequences))
	for s in sequences:
		seq=sequences[s].seq
		lSeq=len(seq)
...
##############################################################################
def main(fastaFile,matFile,output,matname):
	if fastaFile != None or matFile != None :
		...
		if valMat == 0 :
			...
			print "  Calculation of scores and densities"
                        SeqToDensities(fastaFile,matRes,FEAT[2],output,matname,name)
...
if __name__ == "__main__": 
	...
	parser.add_argument("-o","--output", type=str, help='give output file path and name')
        parser.add_argument("--matrix_name", "-matname",action='store_true', default= False)
	...
	output = args.output
	main(fastaFile,matFile,output,matname)
